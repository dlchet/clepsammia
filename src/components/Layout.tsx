...
const TestToggle = () => (
  <authContext.Consumer>
    {({ counter, setObj }) => (
      <button onClick={() => setObj({ counter: counter ?? 0 + 1 })}>
        increment counter
      </button>
    )}
  </authContext.Consumer>
                      ...
                      <Menu as="div" className="relative z-50">
                        <Menu.Button className="flex items-center space-x-px group">
                          <div className="shrink-0 flex items-center justify-center rounded-full overflow-hidden relative bg-gray-200 w-9 h-9">
                            <IoIosPerson className="text-gray-400 w-6 h-6" />
                          </div>
                          <IoIosArrowDown className="w-5 h-5 shrink-0 text-gray-500 group-hover:text-current" />
                        </Menu.Button>
                        <Transition
                          as={Fragment}
